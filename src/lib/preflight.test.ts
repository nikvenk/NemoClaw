...
describe("assessHost", () => {
  ...
  // when the daemon is unreachable — e.g. after `colima stop`. Preflight used
  // to treat any non-empty output as "daemon reachable".
  ...
  it("reports docker unreachable for zero-value JSON even without ServerErrors", () => {
    ...
    // unreachable (empty ServerVersion is the canonical tell).
  ...
  it("reports docker unreachable when output is not valid JSON object", () => {
    const result = assessHost({
      platform: "darwin",
      env: {},
  ...
  it("falls back to non-empty heuristic for non-JSON output (older CLI / test stubs)", () => {
    ...
    // dockerInfoOutput — real CLI paths always emit JSON under `--format`.
    const result = assessHost({
      platform: "darwin",
      env: {},
