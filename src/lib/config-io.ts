...
function buildRemediation(): string {
  ...
  return [
    ...
    `    sudo chown -R $(whoami) ${formatShellToken(nemoclawDir)}`,
    "    # or recreate it if it was created by another user:",
    `    ${joinShellWords(["sudo", "rm", "-rf", nemoclawDir])} && nemoclaw onboard`,
    "",
    "    # If sudo is unavailable, move the bad config aside from a writable HOME:",
    `    ${joinShellWords(["mv", nemoclawDir, backupDir])} && nemoclaw onboard`,
    "    # or, if you already own the directory, remove it without sudo:",
    `    ${joinShellWords(["rm", "-rf", nemoclawDir])} && nemoclaw onboard`,
    "",
    "    # If HOME itself is not writable, start NemoClaw with a writable HOME:",
    `    ${joinShellWords(["mkdir", "-p", recoveryHome])} && ${buildShellAssignment("HOME", recoveryHome)} nemoclaw onboard`,
    "",
