...
export function buildPatchDockerfile(snapshotter: SnapshotterChoice): string {
  ...
  //
  // - We can't `apt-get install fuse-overlayfs` directly into the cluster
  //   image: its base ships BusyBox `tar` rather than GNU `tar`, so
  //   `dpkg-deb` fails to extract any `.deb` (it invokes tar with options
  //   BusyBox doesn't recognize, e.g. `--warning=no-timestamp`).
  //
  // - We can't `RUN curl …` to download the upstream static binary in-place:
  //   the cluster image doesn't ship `curl` either (the upstream Dockerfile
  //   pulls curl in throwaway builder stages and never copies it into the
  //   final layer). `RUN curl` exits 127.
  //
  // The only path that actually works is a tiny Alpine builder stage that
  // fetches the official static binary from containers/fuse-overlayfs, then
  // a single `COPY --from` into the cluster image. The static binary
  // bundles libfuse, so we don't need libfuse3-3 or any other apt package.
  return [
    ...
    "",
    "FROM alpine:3.20 AS bin-fetcher",
    ...
    "      -o /tmp/fuse-overlayfs; \\",
    "    chmod +x /tmp/fuse-overlayfs; \\",
    "    /tmp/fuse-overlayfs --version",
    "",
    "FROM ${UPSTREAM}",
    "USER root",
