...
const { listGatewayDockerVolumes: listGatewayVolumes } = require("./gateway-volumes");
const {
  DASHBOARD_PORT,
  GATEWAY_PORT,
  VLLM_PORT,
...
const onboardOllamaProxy = require("./onboard-ollama-proxy");
const {
...
function removeGatewayDockerVolumes(opts: { suppressOutput?: boolean } = {}): void {
  const volumes = listGatewayVolumes(GATEWAY_NAME, runCapture);
  if (volumes.length === 0) {
    return;
  }
...
async function setupNim(gpu: ReturnType<typeof nim.detectGpu>): Promise<{
  ...
  // Probe the real binary by exit status so stderr from a failed launch does
  // not make Ollama look installed.
  const hasOllama =
    runFile("ollama", ["--version"], {
      ignoreError: true,
      stdio: "ignore",
      suppressOutput: true,
    }).status === 0;
  ...
  if ((hasOllama || ollamaRunning) && process.platform !== "win32") {
    options.push({
      key: "ollama",
      label:
...
/**
 * Pull the gateway auth token from the running sandbox so onboard can print
 * copy-paste Control UI URLs with #token= (same idea as nemoclaw-start.sh).
 * Prefer the root-only runtime token when available, then fall back to the
 * baked OpenClaw config for non-root or older images.
 */
function fetchGatewayAuthTokenFromSandbox(sandboxName: string): string | null {
  // 1. Root mode: kubectl exec reads gateway:gateway 0400 runtime token.
  try {
  ...
  // 2. Fallback: download openclaw.json and read gateway.auth.token.
