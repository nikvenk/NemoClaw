...
const FILES_TO_STAGE = [
  ...
  PLUGIN_PACKAGE_JSON,
  BLUEPRINT_YAML,
  // DOCS_CONF reads the version dynamically from DOCS_VERSIONS_JSON at build time.
  // DOCS_PROJECT_JSON is regenerated by conf.py at build time.
  DOCS_VERSIONS_JSON,
  INSTALL_SH,
...
function main(): void {
  ...
  // conf.py reads the version dynamically from versions1.json at build time.
  // project.json is regenerated by conf.py at build time.
...
function verifyVersionState(version: string, docsPublicUrl: string, docsDisplayVersion: string): void {
  ...
  // conf.py reads the version dynamically from versions1.json; project.json
  // is regenerated at build time.  Only versions1.json needs verification.
