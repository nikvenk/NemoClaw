...
# Unique per build, available as an env var in subsequent layers. Pass
# --build-arg NEMOCLAW_BUILD_ID=$(date +%s) to force a rebuild when needed.
...
# Config generation logic lives in scripts/generate-openclaw-config.py —
# see that file for the full list of env vars and derivation rules.
